...
const MENU_ITEMS: MenuItem[] = [
  { id: 'collector-home', label: 'Dashboard', icon: 'dashboard', enabled: true },
  { id: 'today-route', label: 'Rute Pickup', icon: 'route', enabled: true, badge: 'Aktif' },
  { id: 'pickup-history', label: 'Riwayat Pickup', icon: 'history', enabled: true, badge: 'Aktif' },
  { id: 'inventory', label: 'Inventory', icon: 'inventory_2', enabled: false },
  { id: 'earnings', label: 'Pendapatan', icon: 'payments', enabled: false },
...
export function CollectorLayout({
  ...
  const { profile: collectorProfile } = useCollectorProfile(authProfile?.id)

  const fullName = collectorProfile?.full_name ?? authProfile?.full_name ?? 'Pengepul'
  ...
  const vehicleLabel = collectorProfile?.vehicle_type
